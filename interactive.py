...
ATTACK_CHAIN_PROMPT_PATH = BASE_DIR / "prompts" / "attack_chain_prompt.txt"
...
def build_attack_chain_message(
    target_system: dict,
    quality_result: dict,
    risk_report: str,
) -> str:
    ...
    return (
        "Add a defensive Attack Chain Analysis section to the following enhanced report.\n\n"
        "Generated target JSON:\n"
        f"{target_json}\n\n"
        "JSON quality result:\n"
        f"{quality_json}\n\n"
        ...
        f"{risk_report}\n\n"
        "Return only the full markdown report with the added Attack Chain Analysis section."
    )


def add_attack_chain_analysis(
    model_name: str,
    target_system: dict,
    quality_result: dict,
    risk_report: str,
    ...
    attack_chain_prompt = load_text_file(ATTACK_CHAIN_PROMPT_PATH)

    user_message = build_attack_chain_message(
        target_system=target_system,
        quality_result=quality_result,
        risk_report=risk_report,
    )
...
def save_outputs(
    ...
    enhanced_risk_report: Optional[str] = None,
) -> dict:
    ...
    enhanced_report_path = OUTPUT_DIR / f"enhanced_risk_report_{model_file_name}_{timestamp}.md"
    ...
    if enhanced_risk_report is not None:
        enhanced_report_path.write_text(enhanced_risk_report, encoding="utf-8")
...
Model: {model_name}
Timestamp: {timestamp}
Input mode: natural language -> generated JSON -> normalized JSON -> enriched JSON -> validated JSON -> completeness check -> risk report -> risk normalization -> mitigation enhancement -> attack chain analysis -> report validation
-->
...
"""
    ...
    if enhanced_risk_report is not None:
        result["enhanced_report_path"] = str(enhanced_report_path)
...
def run_virtual_hacker_analysis(
    ...
    try:
        ...
        final_risk_report = add_attack_chain_analysis(
            model_name=model_name,
            target_system=target_system,
            quality_result=quality_result,
        ...
        return {
            ...
            "enhanced_report_path": output_paths.get("enhanced_report_path"),
...
def main() -> None:
    ...
    if result.get("enhanced_report_path"):
        print(f"Enhanced risk report saved in: {result['enhanced_report_path']}")

    if result.get("report_validation_path"):
        print(f"Report validation saved in: {result['report_validation_path']}")

    print(f"Final risk report with attack chains saved in: {result['report_path']}")
